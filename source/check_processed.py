...
for i in range(len(T1w_list)):
    ...
    seg_path = Path(os.path.join(derivatives_dir, f'sub-{subjectID}', f'ses-{sessionID}', 'anat', f'sub-{subjectID}_ses-{sessionID}_space-T1w_seg.nii.gz'))
    ...
    if seg_path in seg_list:
        # write subect and session IDs in the dataframe
...
df_seg_processed.to_csv(os.path.join(args.output_directory, "samseg_cross_processed.csv"), index=False)